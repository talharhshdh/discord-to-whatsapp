    ...
    <div className="space-y-5">
      ...
      {results.length > 0 && (
        <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 gap-4">
          {results.map(m => (
            <div key={m.tmdbId} className="glass glass-hover rounded-xl overflow-hidden flex flex-col">
              {m.posterUrl ? (
                <img src={m.posterUrl} alt={m.title} className="w-full h-48 sm:h-56 object-cover" />
              ) : (
                <div className="w-full h-48 sm:h-56 bg-white/[0.04] flex items-center justify-center text-4xl">🎬</div>
              )}
              <div className="p-3 space-y-2 flex-1 flex flex-col">
                ...
                {m.overview && (
                  <p className="text-[11px] text-white/40 line-clamp-2 flex-1">{m.overview}</p>
                )}
                <a href={m.watchUrl} target="_blank" rel="noopener noreferrer"
                  className="mt-auto block text-center py-2 rounded-lg bg-[#6c63ff]/20 border border-[#6c63ff]/30 text-[#9d97ff] text-xs font-medium hover:bg-[#6c63ff]/30 transition-all">
                  ▶ Watch Now
                </a>
              </div>
            </div>
